...
describe("MockAdapter", () => {
  ...
  describe("Failure simulation", () => {
    it("should fail operations for specific keys", async () => {
      ...
      await expect(adapter.get("fail-key")).rejects.toThrow(
        "Mock failure for key: fail-key",
      );
      await expect(
        adapter.set("fail-key", new Uint8Array([1])),
      ).rejects.toThrow();
  ...
  describe("Quota simulation", () => {
    ...
    it("should allow updating existing keys without checking quota", async () => {
      ...
      await expect(
        adapter.set("key1", new Uint8Array([1, 2, 3])),
