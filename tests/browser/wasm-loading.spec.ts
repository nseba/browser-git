  ...
  test.describe('WASM Instantiation', () => {
    test('should instantiate simple WASM module', async ({ page }) => {
      ...
      const result = await page.evaluate(async () => {
        ...
        try {
          ...
          const add = (module.instance.exports as any).add as (a: number, b: number) => number;
    ...
    test('should handle WASM memory growth', async ({ page }) => {
      ...
      const result = await page.evaluate(async () => {
        ...
        const wasmBytes = new Uint8Array([
          0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00,
          0x05, 0x04, 0x01, 0x01, 0x01, 0x0a, 0x07, 0x0a,
          0x01, 0x06, 0x6d, 0x65, 0x6d, 0x6f, 0x72, 0x79,
          0x02, 0x00
        ]);

        try {
          const module = await WebAssembly.instantiate(wasmBytes);
          const memory = (module.instance.exports as any).memory as WebAssembly.Memory;
    ...
    test('should measure data transfer JS<->WASM', async ({ page }) => {
      ...
      const result = await page.evaluate(async () => {
        ...
        return {
          ...
          writeMBps: Math.round((Math.min(dataSize, buffer.length) / (1024 * 1024)) / (writeDuration / 1000)),
          readMBps: Math.round((Math.min(dataSize, buffer.length) / (1024 * 1024)) / (readDuration / 1000))
        };
      });
    ...
    test('should report WASM features', async ({ page, browserName }) => {
      ...
      const features = await page.evaluate(() => {
        return {
          ...
          hasCompileStreaming: typeof WebAssembly.compileStreaming === 'function',
